<template>
  <div class="main-div">
    ...
    <div v-if="randomString !== 'QNkvcP'" class="deleteButton" @click="askDelete">Delete Room</div>
  ...
  watch: {
    messages() {
      if (this.messages[this.messages.length - 1].username === this.username) {
        ...
        const notification = new Notification(`${this.messages[this.messages.length - 1].username} sent a new message to ${this.roomName}`, { body: this.messages[this.messages.length - 1].message });
      }
    },
    newMessage() {
